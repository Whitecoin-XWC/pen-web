<template>
  <div class="dialog-container">
    <el-dialog :close-on-click-modal="false" :title="$t('转移')" :visible.sync="isShowTransferNft" :before-close="handleClose">
      <el-form label-width="80px" label-position="top">
        <el-form-item :label="$t('目标地址：')">
          <el-input v-model="address" :placeholder="$t('请输入目标地址')"></el-input>
        </el-form-item>
        <div v-if="transferPrice" class="content">{{ $t('费用：') }}{{ transferPrice }}{{ coin }}</div>
        <div class="content2">{{ $t('余额：') }}{{ balanceObj.balances }}{{ balanceObj.name }}</div>

        <el-form-item class="edit-btns">
          <el-button @click="handleClose">{{ $t('取消') }}</el-button>
          <el-button class="submit-btn" type="primary" @click="onSubmit">{{ $t('确定') }}</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
</template>

<script>
import { mapState } from 'vuex'
import http from '../api/http'
export default {
  props: {
    tokenId: {
      type: String,
      default: null,
    },
    copyrightFee: {
      type: String,
      default: null,
    },
  },
  data() {
    return {
      address: '',
      transferPrice: null,
      coin: null,
      balanceObj: {
        balances: 0,
        name: 'XWC',
      },
    }
  },
  computed: {
    ...mapState('global', ['isShowTransferNft', 'coinList']),
  },
  mounted() {
    this.queryNftTransferPrice()
    console.log('coinList', this.coinList)
  },
  methods: {
    // 查询NFT转移费
    async queryNftTransferPrice() {
      const res = await this.$wallet.getTransferFee(this.tokenId, this.copyrightFee)
      console.log('查询NFT转移费', res)
      if (res.code === '0') {
        this.transferPrice = res.data.price
        this.coin = res.data.name
        if (this.coin === '') {
          this.balanceObj = this.coinList.XWC
        } else {
          this.balanceObj = this.coinList[this.coin]
        }
      }
    },
    handleClose() {
      this.$store.commit('global/set_state', { isShowTransferNft: false })
    },
    async onSubmit() {
      if (!this.address.trim().length) {
        this.$message.error(this.$t('请输入目标地址'))
        return false
      }
      const res = await this.$wallet.transferNFT(this.tokenId, this.address, this.transferPrice)
      if (res.code === '0') {
        const res1 = await http(this.$axios).transferNft({ tokenId: this.tokenId, userAddress: this.address, tractionId: res.data.trxid })
        if (res1.code === 200) {
          this.$message.success(this.$t('转移成功'))
          this.$parent.getNftDetails()
          this.$store.commit('global/set_state', { isShowTransferNft: false })
        }
      }
    },
  },
}
</script>

<style scoped lang="scss">
.dialog-container {
  position: relative;

  /deep/ .el-dialog {
    border-radius: 6px !important;
    width: 25% !important;
  }
  /deep/.el-dialog__title {
    font-size: 16px;
    font-weight: 700;
    color: #000000d9;
    line-height: 24px;
  }
  /deep/.el-dialog__header {
    border-bottom: 1px solid #0000000f;
  }

  /deep/.el-dialog__body {
    padding: 20px 28px 10px 28px;
  }
  .edit-btns {
    text-align: right;
    margin-top: 38px;
    margin-bottom: 0;
    padding-top: 10px;
    &::before {
      display: inline-block;
      content: '';
      height: 1px;
      width: 100%;
      position: absolute;
      background: #0000000f;
      bottom: 58px;
      left: 0;
    }
    .submit-btn {
      color: #ffffff;
      background-color: black;
      border-color: black;
    }
  }
  /deep/.el-form-item__label {
    padding-bottom: 0;
  }

  .content {
    height: 30px;
    font-size: 14px;
    font-weight: 400;
    color: #000000;
    line-height: 22px;
  }
  .content2 {
    height: 20px;
    font-size: 14px;
    font-weight: 600;
    color: #43cee2;
    line-height: 14px;
    text-align: right;
  }
}
</style>
