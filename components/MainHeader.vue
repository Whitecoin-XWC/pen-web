<template>
  <div>
    <div class="header">
      <div class="header-l">
        <!-- 头像 -->
        <div class="head-portrait" @click="toHome">
          <img src="../assets/img/header/portrait.png" alt="" />
        </div>
        <!-- 搜索框 -->
        <div class="search">
          <el-input v-model="searchMsg" :placeholder="$t('搜索')" @keyup.13.native="searchList"> </el-input>
          <i class="search-icon el-icon-search" @click="searchList"></i>
        </div>
      </div>
      <div class="header-r">
        <div class="dropDownLang">
          <div class="nowLang" @mouseover="dropDownIsShow = true" @mouseout="dropDownIsShow = false">
            <a v-cloak src="javaScript:void(0)">{{ nowLang }}<span class="iconfont icon-arrow"></span></a>
            <div v-show="dropDownIsShow" class="lang-menu">
              <li :class="nowLang == '简体中文' ? 'active' : ''" @click="switchLang('cn', '简体中文')">简体中文</li>
              <li :class="nowLang == 'English' ? 'active' : ''" @click="switchLang('en', 'English')">English</li>
            </div>
          </div>
        </div>
        <!-- 消息通知图标 -->
        <div class="msg-box" @click="openNotice">
          <span class="msg-num">{{ msgNum }}</span>
          <i class="el-icon-bell"></i>
        </div>
        <!-- 账户 -->
        <div class="user-box" @click="toPersonal">{{ address }}</div>
        <div class="issue-btn" @click="toPublish">{{ $t('发行') }}</div>
      </div>
      <div v-if="isShowSearchBox" class="search-box">
        <el-tabs v-model="activeName" @tab-click="handleClick">
          <el-tab-pane :label="$t('创作者')" name="Artist">
            <div class="pane-scroll">
              <p v-if="!usersList.length" class="not-data">暂无数据</p>
              <div v-if="usersList.length" class="pane-box">
                <p v-for="(item, index) in usersList" :key="index" class="artist-text">{{ item.nickName }}</p>
              </div>
            </div>
          </el-tab-pane>
          <el-tab-pane :label="$t('名称')" name="Name">
            <div class="pane-scroll">
              <p v-if="!filesList.length" class="not-data">暂无数据</p>
              <div v-if="filesList.length" class="pane-box">
                <div v-for="(item, index) in filesList" :key="index" class="name-item">
                  <img class="name-img" :src="item.fileName" alt="" />
                  <div>
                    <p class="name-p1">{{ item.fileTitle }}</p>
                    <p class="name-p2">{{ $t('创作者') }}：大帅逼</p>
                  </div>
                </div>
              </div>
            </div>
          </el-tab-pane>
        </el-tabs>
      </div>
    </div>
    <DialogNotice v-if="isShowNoticeDialog" />
    <DialogDownload v-if="isShowDownloadDialog" />
    <DialogRelink />
  </div>
</template>

<script>
import { mapState } from 'vuex'
import http from '../api/http'
export default {
  props: {},
  data() {
    return {
      edition: '',
      searchMsg: '',
      msgNum: 1,
      nowLang: '简体中文',
      walletAddress: {},
      dropDownIsShow: false,
      isExtInstalled: false,
      isShowSearchBox: false,
      activeName: 'Artist',
      filesList: [],
      usersList: [],
    }
  },
  computed: {
    ...mapState('global', ['isShowNoticeDialog', 'isShowDownloadDialog', 'isShowRelinkDialog', 'address']),
  },
  watch: {
    searchMsg(newVal, oldVal) {
      if (!newVal.trim().length) {
        this.isShowSearchBox = false
        this.activeName = 'Artist'
        this.filesList = []
        this.usersList = []
      } else {
        this.isShowSearchBox = true
      }
    },
  },
  mounted() {
    this.nowLang = this.$cookies.get('_lang') === 'cn' ? '简体中文' : 'English'
    this.initWallet()
  },
  methods: {
    handleClick() {},

    async searchList() {
      if (!this.searchMsg.length) {
        this.$message.error('请输入搜索关键字')
        return false
      }
      const res = await http(this.$axios).getSearchNftList({ keyWord: this.searchMsg })
      if (res.code === 200 && res.data) {
        this.usersList = res.data.users
        this.filesList = res.data.files
        this.filesList.forEach((item) => {
          item.fileName = `${res.data.imagUrl}${item.fileName}`
        })
        if (!this.usersList.length && this.filesList.length) {
          this.activeName = 'Name'
        } else {
          this.activeName = 'Artist'
        }
      }
    },
    async initWallet() {
      this.isExtInstalled = this.$wallet.checkInstalled()
      if (this.isExtInstalled) {
        const res = await this.$wallet.connectWallet()
        console.log('========', res)
        if (res.code === '0') {
          // 钱包是否初始化
          this.isWalletInited = res.data.isWalletInited
          // 钱包对象
          this.walletAddress = res.data.walletAddress
          this.$store.commit('global/set_state', { address: this.walletAddress.address })
        } else {
          // 提示重新链接钱包
          this.$store.commit('global/set_state', { isShowRelinkDialog: true })
        }
        // 余额
        // const result = await this.$wallet.getCoinList()
        // if (result.code === '0') {
        //   this.$store.commit('global/set_state', { coinList: result.data })
        // }
      } else {
        // 弹窗提示下载钱包插件
        this.$store.commit('global/set_state', { isShowDownloadDialog: true })
      }
    },
    switchLang(lang, type) {
      if (this.$cookies.get('_lang') !== lang) {
        this.$cookies.set('_lang', lang)
        this.$store.commit('global/set_locale', lang)
        this.$i18n.locale = lang
        this.nowLang = type
      }
    },
    openNotice() {
      this.$store.commit('global/set_state', { isShowNoticeDialog: !this.isShowNoticeDialog })
    },
    toPublish() {
      this.$router.push('/publish')
    },
    toHome() {
      this.$router.push('/home')
    },
    toPersonal() {
      this.$router.push('/personal')
    },
  },
}
</script>

<style scoped lang="scss">
.header {
  position: relative;
  width: 100%;
  height: 117px;
  background-color: #ffffff;
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 0 27px;
  border-bottom: 2px solid #e6e6e6;
}
.search-box {
  position: absolute;
  left: 87px;
  top: 90px;
  width: 455px;
  height: 202px;
  background: #ffffff;
  box-shadow: 0 9px 28px 8px rgba(0, 0, 0, 0.05), 0 6px 16px 0 rgba(0, 0, 0, 0.08), 0 3px 6px -4px rgba(0, 0, 0, 0.12);
  .pane-scroll {
    width: 100%;
    height: 140px;
    overflow: hidden;
    .not-data {
      line-height: 140px;
      text-align: center;
      font-weight: 400;
      color: #00000080;
      font-size: 20px;
    }
    .pane-box {
      width: 100%;
      height: 100%;
      box-sizing: border-box;
      overflow-y: scroll;
      .artist-text {
        font-size: 14px;
        font-weight: 400;
        color: #000000a6;
        line-height: 20px;
        cursor: pointer;
        text-indent: 38px;
        &:hover {
          background-color: #c1909040;
        }
      }
      .name-item {
        height: 60px;
        padding: 10px 38px;
        display: flex;
        .name-img {
          width: 40px;
          height: 40px;
          margin-right: 14px;
        }
        .name-p1 {
          font-size: 16px;
          font-weight: 600;
          color: #000000;
          line-height: 22px;
        }
        .name-p2 {
          font-size: 14px;
          font-weight: 400;
          color: #999999;
          line-height: 20px;
        }
        &:hover {
          background-color: #c1909040;
        }
      }
    }
  }
  /deep/.el-tabs__active-bar {
    background-color: #000000ff;
  }
  /deep/.el-tabs__item {
    padding: 0 75px;
    font-size: 16px;
    font-weight: 400;
    color: #00000040;
    &.is-active {
      font-weight: 600;
      color: #000000d9;
    }
  }
  /deep/.el-tabs__nav-scroll {
    padding: 0 38px;
  }
}
.header-r,
.header-l {
  display: flex;
  align-items: center;
}
.head-portrait {
  width: 50px;
  height: 50px;
  border-radius: 50%;
  img {
    width: 100%;
    height: 100%;
  }
}
.search {
  width: 455px;
  height: 50px;
  position: relative;
  margin-left: 11px;
  /deep/.el-input {
    width: 100%;
    height: 100%;
    .el-input__inner {
      height: 100%;
    }
  }
  .search-icon {
    display: inline-block;
    width: 22px;
    height: 21px;
    position: absolute;
    right: 15px;
    top: 15px;
    color: #000000;
    cursor: pointer;
  }
}
.msg-box {
  position: relative;
  cursor: pointer;
  .msg-num {
    position: absolute;
    display: inline-block;
    height: 22px;
    padding: 0 5px;
    font-size: 12px;
    background: #ff6262;
    border-radius: 50%;
    border: 2px solid #ffffff;
    color: #ffffff;
    text-align: center;
    top: -2px;
    right: -3px;
  }
  .el-icon-bell {
    font-size: 30px;
  }
}
.user-box {
  width: 426px;
  height: 50px;
  line-height: 50px;
  background: #ffffff;
  box-shadow: 0 5px 8px 0 rgba(0, 0, 0, 0.06);
  border-radius: 4px;
  margin-left: 20px;
  cursor: pointer;
  text-indent: 24px;
}
.issue-btn {
  width: 100px;
  height: 50px;
  background: linear-gradient(180deg, #000000 0%, #000000 100%);
  box-shadow: 0 5px 8px 0 rgba(0, 0, 0, 0.06);
  border-radius: 4px;
  border: 1px solid #ebebeb;
  color: #ffffff;
  font-size: 18px;
  font-weight: 600;
  line-height: 50px;
  text-align: center;
  margin-left: 20px;
  cursor: pointer;
}
.dropDownLang {
  position: relative;
  margin-right: 10px;
  .nowLang {
    cursor: pointer;
    .icon-arrow {
      font-size: 6px;
    }
    a {
      display: flex;
      justify-content: center;
      align-items: center;
      width: 95px;
      height: 32px;
      border-radius: 16px;
      font-size: 14px;
      font-weight: 400;
      color: #606266;
      span {
        margin-left: 10px;
      }
    }
  }
  .lang-menu {
    position: absolute;
    z-index: 999;
    background-color: #1d1d4d;
    width: 100%;
    border-radius: 10px;
    padding: 8px 0;
    margin-top: 6px;
    &::after {
      display: block;
      content: '';
      height: 6px;
      width: 100%;
      position: absolute;
      top: -6px;
      left: 0;
    }

    &::before {
      display: block;
      width: 0;
      content: '';
      border-width: 6px 6px 6px 6px;
      border-style: solid;
      border-color: transparent transparent #1d1d4d transparent;
      position: absolute;
      left: 50%;
      top: -12px;
      transform: translateX(-50%);
    }
    li {
      padding: 5px 6px;
      text-align: center;
      color: white;
      list-style: none;
      font-size: 16px;
      &:hover {
        background-color: #101032;
        color: #7777bc;
      }

      &.active {
        background-color: #16163f;
        color: #7777bc;
      }
    }
  }
}
</style>
